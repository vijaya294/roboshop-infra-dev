...
module "records" {
  ...
  zone_name = var.zone_name #daws76s.icu
  records = [
    {
      name    = "roboshop-${var.environment}" # roboshop-dev.daws76s.icu
      type    = "A"
      alias   = {
...
resource "aws_lb_listener_rule" "frontend" {
  ...
  condition {
    host_header {
      values = ["roboshop-${var.environment}.${var.zone_name}"] #roboshop-dev.daws76s.icu
    }
  }
}